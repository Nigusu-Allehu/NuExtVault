...
public sealed class OwnershipAndCapabilityContractTests
{
        ...
        var violations = capabilities.SelectMany(type => type.GetMethods().SelectMany(method =>
        {
                         ...
                         .SelectMany(Expand))
            {
                ...
                if (name is "System.IO.Stream" or "System.IServiceProvider" ||
                    ...
                    name.StartsWith("NuGet.TestServer.Kernel.", StringComparison.Ordinal) ||
    ...
    public void Official_modules_and_separately_compiled_fixture_conform_to_the_public_sdk()
    {
        ...
        var official = Assembly.Load("NuGet.TestServer.Extensions.Official");
        var fixture = typeof(NuGet.TestServer.SdkFixture.FlavorsExtension).Assembly;

        Assert.Equal(
            [sdk],
            official.GetReferencedAssemblies()
                .Select(reference => reference.Name)
                .Where(name => name?.StartsWith("NuGet.TestServer.Extensions.", StringComparison.Ordinal) == true)
                .Except(["NuGet.TestServer.Extensions.Official"])
                .Select(name => name!)
