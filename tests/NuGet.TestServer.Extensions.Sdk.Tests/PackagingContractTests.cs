...
public sealed class PackagingContractTests
{
    ...
    public async Task Sdk_and_testkit_projects_pack_only_net10_contract_assets()
    {
        ...
        var sdk = PackAsync("NuGet.TestServer.Extensions.Sdk", output);
        var testKit = PackAsync("NuGet.TestServer.Extensions.TestKit", output);
    ...
    public void Package_projects_are_net10_only_and_do_not_bundle_host_implementation()
    {
                 ...
                 })
        {
            ...
            Assert.DoesNotContain(
                references,
                reference => reference.Contains("NuGet.TestServer.Kernel", StringComparison.Ordinal));
            Assert.DoesNotContain(
                references,
    ...
    private static void AssertPackage(string path, string[] required)
    {
        ...
        Assert.DoesNotContain(
            files,
            entry => entry.EndsWith("NuGet.TestServer.dll", StringComparison.Ordinal) ||
                     entry.EndsWith("NuGet.TestServer.Kernel.dll", StringComparison.Ordinal) ||
                     entry.EndsWith("NuGet.TestServer.Extensions.Official.dll", StringComparison.Ordinal));
