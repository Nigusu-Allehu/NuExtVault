...
public sealed class OperationRouteCoverageTests
{
    ...
    public void Production_hosts_do_not_map_test_control_routes_or_owners()
    {
        ...
        Assert.DoesNotContain(
            ...
            operationId => operationId.StartsWith("NuExtVault.Control.", StringComparison.Ordinal));
        Assert.DoesNotContain(
            host.Registry.Registrations,
            registration => registration.Id.Value.StartsWith("NuExtVault.Control.", StringComparison.Ordinal));
        Assert.Contains(
            routes,
