...
public sealed class ExtensionModuleFitnessTests
{
    ...
    public void The_kernel_has_no_compile_time_knowledge_of_the_conformance_module()
    {
        ...
        Assert.DoesNotContain("NuExtVault.RouteFixture", references);
    ...
    public void The_conformance_module_references_only_the_extension_sdk()
    {
        var project = File.ReadAllText(Path.Combine(
            RepositoryRoot,
            "tests",
            "NuExtVault.RouteFixture",
            "NuExtVault.RouteFixture.csproj"));
        ...
        Assert.Equal(
            ["NuExtVault.Extensions.Sdk.csproj"],
            references);
        ...
        var offenders = EnumerateSourceFiles(Path.Combine(
            ...
            .Where(file => Regex.IsMatch(
                ...
                @"using\s+NuExtVault\.(?!Extensions\.Sdk)"))
            .Select(file => Path.GetRelativePath(RepositoryRoot, file))
            .ToArray();
    ...
    public void Owner_shaped_kernel_types_never_appear_in_extension_facing_capabilities()
    {
        string[] banned =
        [
            "NuExtVault.Kernel.OperationExecutionContext",
            "NuExtVault.Packages.TestPackage",
            "NuExtVault.Packages.IPackageStore",
            "NuExtVault.Packages.PackageRepositoryMetadata",
            "NuExtVault.Operations.StorageBackupManifest",
            "NuExtVault.Faults.FaultRule",
            "NuExtVault.Requests.RequestRecord",
            "NuExtVault.Extensions.Vulnerabilities.VulnerabilitySnapshot",
            "System.IO.Stream",
    ...
    internal static string OfficialExtensionRoot =>
        Path.Combine(RepositoryRoot, "src", "NuExtVault.Extensions.Official");
...
internal static class ExtensionFacingCapabilities
{
    ...
    {
        var assembly = typeof(NuExtVault.Extensions.Official.OfficialExtensionModules)
            .Assembly;
    ...
    public static bool IsSerializableContractType(Type type)
    {
        ...
        return namespaceName == "NuExtVault.Extensions.Sdk" ||
    ...
    private static IEnumerable<Type> Expand(Type type, HashSet<Type> visited)
    {
        ...
        if (type.Namespace == "NuExtVault.Extensions.Sdk" &&
