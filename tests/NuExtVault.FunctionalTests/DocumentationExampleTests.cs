...
public sealed class DocumentationExampleTests
{
    ...
    public void Reference_examples_match_implemented_contracts()
    {
        ...
        Assert.StartsWith("$env:NUEXTVAULT_IDENTITIES = \"{{IDENTITY_JSON}}\"",
            productionStart, StringComparison.Ordinal);
        Assert.Contains("& \"{{TOOL_COMMAND}}\" start --production",
            productionStart, StringComparison.Ordinal);
        Assert.Contains("--identity-config-env NUEXTVAULT_IDENTITIES",
            productionStart, StringComparison.Ordinal);
    ...
    public async Task Public_sdk_example_compiles_and_runs_conformance()
    {
        ...
        await File.WriteAllTextAsync(
            ...
            <Project Sdk="Microsoft.NET.Sdk">
              ...
              <ItemGroup>
                <ProjectReference Include="{EscapeXml(Path.Combine(RepositoryRoot, "src", "NuExtVault.Extensions.Sdk", "NuExtVault.Extensions.Sdk.csproj"))}" />
                <ProjectReference Include="{EscapeXml(Path.Combine(RepositoryRoot, "src", "NuExtVault.Extensions.TestKit", "NuExtVault.Extensions.TestKit.csproj"))}" />
                <ProjectReference Include="{EscapeXml(Path.Combine(RepositoryRoot, "tests", "NuExtVault.SdkFixture", "NuExtVault.SdkFixture.csproj"))}" />
              </ItemGroup>
            </Project>
            """);
        ...
        foreach (var id in new[] { "user-03-generated-package", "user-03-parallel-isolation" })
        {
            ...
            await File.WriteAllTextAsync(
                ...
                <Project Sdk="Microsoft.NET.Sdk">
                  ...
                  <ItemGroup>
                    <ProjectReference Include="{EscapeXml(Path.Combine(RepositoryRoot, "src", "NuExtVault", "NuExtVault.csproj"))}" />
    ...
    public async Task Http_and_control_examples_execute_exact_displayed_scripts()
    {
        ...
        await server.Packages.AddAsync(
            TestPackageBuilder.Create("NuExtVault.Docs.Workflow", "1.0.0").Build());
        ...
        Assert.Null(await server.Packages.FindAsync("NuExtVault.Docs.Workflow", "1.0.0"));
    ...
    public async Task Production_route_example_executes_exact_displayed_script()
    {
        ...
        await using var server = await NuExtVaultHost.StartProductionAsync(security);
    ...
    public async Task Push_restore_and_config_examples_execute_exact_displayed_text()
    {
        await using var server = await NuExtVaultHost.StartAsync();
        using var directory = TemporaryDirectory.Create();
        var packagePath = Path.Combine(directory.Path, "NuExtVault.Docs.Workflow.1.0.0.nupkg");
        using (var package = TestPackageBuilder.Create("NuExtVault.Docs.Workflow", "1.0.0").Build())
        {
        ...
        await File.WriteAllTextAsync(
            ...
            <Project Sdk="Microsoft.NET.Sdk">
              ...
              <ItemGroup><PackageReference Include="NuExtVault.Docs.Workflow" Version="1.0.0" /></ItemGroup>
            </Project>
            """);
        var replacements = new Dictionary<string, string>
        {
    ...
    public async Task Cli_pack_install_backup_restore_and_cleanup_examples_execute()
    {
        ...
        await using (var host = await NuExtVaultHost.StartAsync(
            backupStorage,
    ...
    private static async Task<bool> IsListedAsync(NuExtVaultHost server)
    {
        var packages = await server.HttpClient.GetFromJsonAsync<JsonElement>("/__test/packages");
        return packages.EnumerateArray()
            .Single(package =>
                package.GetProperty("id").GetString() == "NuExtVault.Docs.Workflow" &&
    ...
    private static string[] ProductProjectEdges()
    {
        string[] projectPaths =
        [
            "src\\NuExtVault.Extensions.Sdk\\NuExtVault.Extensions.Sdk.csproj",
            "src\\NuExtVault.Kernel\\NuExtVault.Kernel.csproj",
            "src\\NuExtVault.Extensions.Official\\NuExtVault.Extensions.Official.csproj",
            "src\\NuExtVault\\NuExtVault.csproj",
            "src\\NuExtVault.Cli\\NuExtVault.Cli.csproj",
            "src\\NuExtVault.Extensions.TestKit\\NuExtVault.Extensions.TestKit.csproj",
            "src\\NuExtVault.Extensions.PackageStaging\\NuExtVault.Extensions.PackageStaging.csproj"
        ];
    ...
    private static string[] BrokerBackedPublicCapabilities()
    {
        var broker = File.ReadAllText(Path.Combine(
            ...
            "Capabilities",
            "CapabilityBroker.cs"));
        var exportedSdkTypes = typeof(NuExtVault.Extensions.Sdk.IExtensionModule)
            .Assembly
            .GetExportedTypes()
    ...
    private sealed class TemporaryFile(string path) : IDisposable
    {
        ...
        public static TemporaryFile Create(string extension)
        {
            var path = System.IO.Path.Combine(
                ...
                $"nuextvault-docs-{Guid.NewGuid():N}{extension}");
    ...
    private sealed class TemporaryDirectory(string path) : IDisposable
    {
        ...
        public static TemporaryDirectory Create()
        {
            var parent = OperatingSystem.IsMacOS()
                ...
                : System.IO.Path.Combine(System.IO.Path.GetTempPath(), "NuExtVault.DocumentationTests");
