...
public sealed class FlatContainerResourceTests
{
    ...
    public async Task Extracted_flat_container_reads_are_read_your_writes_and_survive_restart()
    {
        ...
        await using (var first = await NuExtVaultHost.StartAsync(directory.Path, PackageTransferLimits.Default))
        {
        ...
        await using var second = await NuExtVaultHost.StartAsync(directory.Path, PackageTransferLimits.Default);
