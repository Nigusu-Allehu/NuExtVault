...
public sealed class PublishAndControlTests
{
    ...
    public async Task Request_body_limit_is_enforced_by_real_kestrel()
    {
        ...
        await using var server = await NuExtVaultHost.StartAsync(limits);
    ...
    {
        await using var server = await NuExtVaultHost.StartAsync(CreateLimits());
    ...
    public async Task Fault_stage_precedes_authentication_and_is_isolated_per_host()
    {
        var authentication = AuthenticationConfiguration.Create(
            ...
            "test-password",
            apiKey: null);
        await using var first = await NuExtVaultHost.StartAsync(authentication);
        await using var second = await NuExtVaultHost.StartAsync(authentication);
