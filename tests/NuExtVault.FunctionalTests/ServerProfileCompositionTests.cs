...
public sealed class ServerProfileCompositionTests
{
    ...
    public async Task Every_programmatic_overload_uses_an_equivalent_profile_composition()
    {
        ...
        var embeddedFactories = new Func<Task<NuExtVaultHost>>[]
        {
            () => NuExtVaultHost.StartAsync(),
            () => NuExtVaultHost.StartAsync(runtimeState),
            () => NuExtVaultHost.StartAsync(authentication),
            () => NuExtVaultHost.StartAsync(limits),
            () => NuExtVaultHost.StartAsync(new SupplyChainOptions()),
            () => NuExtVaultHost.StartAsync(storage.Path, limits),
            () => NuExtVaultHost.StartAsync(snapshot),
            () => NuExtVaultHost.StartAsync(authentication, snapshot),
            () => NuExtVaultHost.StartAsync(authentication, snapshot, limits),
            () => NuExtVaultHost.StartAsync(
                authentication,
                snapshot,
                limits,
                runtimeState),
            () => NuExtVaultHost.StartAsync(ServerMode.Test, authentication),
            () => NuExtVaultHost.StartAsync(ServerMode.Test, authentication, storage.Path),
            () => NuExtVaultHost.StartAsync(ServerMode.Test, authentication, snapshot),
            () => NuExtVaultHost.StartAsync(
                ServerMode.Test,
                authentication,
                snapshot,
        ...
        await using var productionWithStorage = await NuExtVaultHost.StartAsync(
            ServerMode.Production,
            authentication,
            productionStorage.Path);
        await using var productionWithoutStorage = await NuExtVaultHost.StartAsync(
            ServerMode.Production,
            authentication);
        ...
        await using var productionIdentity = await NuExtVaultHost.StartProductionAsync(
            productionSecurity);
    ...
    public async Task Parallel_hosts_keep_profile_configuration_and_state_isolated()
    {
        ...
        var authentication = AuthenticationConfiguration.Create(
            ...
            password: null,
            apiKey: "publish-key");
        var embeddedTask = NuExtVaultHost.StartAsync(
            new RuntimeStateConfiguration(requestHistoryCapacity: 1, faultRuleCapacity: 1));
        var productionTask = NuExtVaultHost.StartAsync(
            ServerMode.Production,
            authentication,
            productionStorage.Path);
    ...
    public async Task One_hundred_parallel_embedded_hosts_keep_mutable_state_isolated()
    {
        var starts = Enumerable.Range(0, 100)
            .Select(_ => NuExtVaultHost.StartAsync())
            .ToArray();
