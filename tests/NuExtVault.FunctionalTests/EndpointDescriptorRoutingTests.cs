...
public sealed class EndpointDescriptorRoutingTests
{
    ...
    public async Task Access_policies_are_enforced_before_request_binding()
    {
        var authentication = AuthenticationConfiguration.Create(
            ...
            password: null,
            apiKey: "control-key");
        await using var server = await NuExtVaultHost.StartAsync(authentication);
    ...
    private static Task<NuExtVaultHost> StartWithFlavorsAsync() => FlavorsHost.StartAsync();
}
