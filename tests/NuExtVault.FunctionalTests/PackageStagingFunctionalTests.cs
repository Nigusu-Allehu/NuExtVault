...
public sealed class PackageStagingFunctionalTests(PackageStagingFunctionalAssetsFixture fixture)
{
    ...
    public async Task The_service_index_advertises_the_staging_resource_with_a_projected_url()
    {
        ...
        var resource = document.RootElement.GetProperty("resources")
            ...
            .Single(entry => (entry.GetProperty("@type").GetString() ?? string.Empty)
                .StartsWith("NuExtVault.PackageStaging.ServiceIndex", StringComparison.Ordinal));
        Assert.Equal(
    ...
    public async Task A_malformed_package_is_rejected_with_redacted_detail()
    {
        ...
        Assert.DoesNotContain("NuExtVault.", detail, StringComparison.Ordinal);
    }
    ...
    private static HttpClient Authorized(NuExtVaultHost server)
    {
    ...
    private static async Task<string> SendOversizedHeadersAsync(NuExtVaultHost server)
    {
    ...
    private static string CreateStorage()
    {
        var path = Path.Combine(
            Path.GetTempPath(),
            "nuextvault-staging-functional",
            Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(path);
        return path;
    }

    private Task<NuExtVaultHost> StartAsync(bool requireApiKey = false)
    {
        var assets = fixture.StagingAssets;
        var (key, trustRoot) = ConformanceAttestationFixture.CreateTrustedKey(publisher: "NuExtVault");
