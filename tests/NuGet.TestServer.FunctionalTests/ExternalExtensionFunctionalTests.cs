...
/// <see cref="NuGetTestServerHost"/> whose profile enables the
/// <c>externalExtensions</c> parameter of <see cref="ServerComposition.Create"/>,
/// loading the real, separately packaged <c>Contoso.NuTestServer.Flavors</c>
/// fixture (tests/NuGet.TestServer.SdkFixture, packed by
...
public sealed class ExternalExtensionFunctionalTests(ExternalExtensionFunctionalAssetsFixture fixture)
{
    ...
    private async Task<NuGetTestServerHost> StartHostWithTrustedFlavorsPackageAsync()
    {
        var (key, trustRoot) = ConformanceAttestationFixture.CreateTrustedKey();
