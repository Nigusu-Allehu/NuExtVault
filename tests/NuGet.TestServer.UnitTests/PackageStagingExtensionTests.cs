...
public sealed class PackageStagingExtensionTests(PackageStagingAssetsFixture fixture)
{
    ...
    public void The_staging_assembly_references_only_the_public_sdk()
    {
        ...
        Assert.Contains("NuGet.TestServer.Extensions.Sdk", references);
        Assert.DoesNotContain("NuGet.TestServer.Kernel", references);
        Assert.DoesNotContain("NuGet.TestServer.Extensions.Official", references);
        Assert.DoesNotContain("NuGet.TestServer", references);
    }
    ...
    public void The_staging_manifest_declares_state_capabilities_and_a_resource_route()
    {
        ...
        Assert.Equal(new RouteIdentity("nutest.staging.list-groups"), contribution.Route);
    }
    ...
    public void The_staging_manifest_binds_uploads_to_streaming_bodies()
    {
        ...
        Assert.Equal(
            ...
            Route(manifest, "nutest.staging.upload-package").Body);
        Assert.Equal(
            RouteBodyBinding.None,
            Route(manifest, "nutest.staging.list-groups").Body);
        Assert.Equal(
            RouteBodyBinding.Bounded,
            Route(manifest, "nutest.staging.create-group").Body);
        Assert.Contains(
            "Idempotency-Key",
            Route(manifest, "nutest.staging.upload-package").Headers);
    }
...
{
    private const string Owner = "NuTest.PackageStaging";
    ...
    public async Task Unfinished_entries_survive_restart_and_committed_ones_do_not_reappear()
    {
        ...
        Assert.Equal(
            ...
            restarted.ReadUnfinished()
                ...
                .ToArray());
        Assert.Equal(7, restarted.Find("NuTest.PackageStaging", "committed")!.StateToken);
    }
