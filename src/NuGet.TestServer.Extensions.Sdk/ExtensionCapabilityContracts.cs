...
internal interface IKernelOutboundHttpCapability
{
    ValueTask<KernelOutboundHttpResponse> SendAsync(
        KernelOutboundHttpRequest request,
        CancellationToken token);
}

internal sealed record KernelOutboundHttpRequest(
    Uri Uri,
    string Method,
    ImmutableDictionary<string, string> Headers,
    long MaximumResponseBytes);

internal sealed record KernelOutboundHttpResponse(
    int statusCode,
    ImmutableDictionary<string, string> headers,
...
/// </summary>
public sealed class CapabilityDeniedException(string extensionId, string capabilityName)
    : InvalidOperationException(
        $"Owner '{extensionId}' was denied undeclared or ungranted capability '{capabilityName}'.")
{
    public string ExtensionId { get; } = extensionId;

    public string CapabilityName { get; } = capabilityName;

    internal string OwnerId => ExtensionId;
}
